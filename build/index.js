...
export class ExpoMutualTls {
     ...
     */
    static async configureP12(keychainService = "client.p12", enableLogging = false) {
        const config = {
     ...
     * @param enableLogging - Enable debug logging
     */
    static async configurePEM(certService = "expo.mtls.client.cert", keyService = "expo.mtls.client.key", enableLogging = false) {
        const config = {
     ...
     */
    static async storeP12(p12Base64, password) {
        const certData = { p12Data: p12Base64, password };
     ...
     */
    static async storePEM(certificate, privateKey, passphrase) {
        const certData = {
            certificate,
            privateKey,
            passphrase,
    ...
    static onCertificateExpiry(listener) {
     ...
     */
    static async parseCertificate(certificateData) {
     ...
     */
    static async parseCertificateP12(p12Base64, password) {
        const certData = { p12Data: p12Base64, password };
     ...
     * @returns Certificate information
     */
    static async parseCertificatePEM(certificate) {
        const certData = {
            certificate,
     ...
     */
    static async connectWebSocket(url, protocols) {
     ...
     */
    static async disconnectWebSocket(connectionId) {
     ...
     */
    static async sendWebSocketMessage(connectionId, message) {
     ...
     */
    static async getWebSocketState(connectionId) {
     ...
     * @returns Event subscription for removing the listener
     */
    static onWebSocketMessage(callback) {
        return ExpoMutualTlsModule.addListener("onWebSocketEvent", (event) => {
            if (event.type === "message")
                callback(event.connectionId, event.data || "");
        });
    }
    /**
     ...
     * @returns Event subscription for removing the listener
     */
    static onWebSocketClose(callback) {
        return ExpoMutualTlsModule.addListener("onWebSocketEvent", (event) => {
            if (event.type === "close") {
     ...
     * @returns Event subscription for removing the listener
     */
    static onWebSocketError(callback) {
        return ExpoMutualTlsModule.addListener("onWebSocketEvent", (event) => {
            if (event.type === "error")
                callback(event.connectionId, event.error || "");
        });
    }
}
